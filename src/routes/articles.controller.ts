...
async function logUpdate(articleId: any, changes: any) {
  ...
  exec('git add updateLog.txt && git commit -m "Logged update for article ${articleId}"', (error, stdout, stderr) => {
    if (error) {
...
export class ArticlesController extends Controller {
  ...
  public async getArticles(): Promise<Article[]> {
    return [/* Insert article interface after extracting from the database */];
  }
  ...
  @Patch("/{id}")
  public async updateArticle(@Path() id: string, @Body() requestBody: Article): Promise<Article> {
    // Logic to update the article

    logUpdate(id, requestBody);  // Assuming requestBody as the source of changes
